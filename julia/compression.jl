...
# Wymiary obrazu - wykryte automatycznie z rozmiaru pliku
total_pixels = length(raw_data) ÷ 3
# Dla standardowego formatu 3:2 (DSLR)
height = 2348
width = 3522
...
# Konwersja UInt8 -> Int dla integer DWT
r_channel = Int.(img_array[1, :, :]')
g_channel = Int.(img_array[2, :, :]')
b_channel = Int.(img_array[3, :, :]')
...
for level in 1:levels
    ...
    # Konwersja Int -> Float64 tylko do wizualizacji
    img_reconstructed = colorview(RGB, 
        normalize_img(Float64.(abs.(X_r)), gamma),
        normalize_img(Float64.(abs.(X_g)), gamma),
        normalize_img(Float64.(abs.(X_b)), gamma))
